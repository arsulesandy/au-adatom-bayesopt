...
Fig.~\ref{fig:bo} shows convergence and the posterior for the best-performing run ($\beta=2.0$). The GP variance is largest near unsampled ridges and deep hollows; the acquisition targets those regions while still refining around the terrace minimum. The global minimum was typically identified by $\sim25$ EMT evaluations (initial five plus roughly 20 BO iterations) in successful runs.
...
\begin{itemize}
    ...
    \item To regenerate the notebook and figures, run:\\
    \texttt{JUPYTER\_CONFIG\_DIR=assignments/project-2b/artifacts/jupyter\_config\\JUPYTER\_DATA\_DIR=assignments/project-2b/artifacts/jupyter\_data\\JUPYTER\_RUNTIME\_DIR=assignments/project-2b/artifacts/jupyter\_runtime\\assignments/project-2b/.venv312/bin/jupyter nbconvert --to notebook --execute --inplace assignments/project-2b/project2b\_bayesopt\_solution.ipynb}
    \item Key metrics are collected in \texttt{assignments/project-2b/artifacts/summary.json}.
\end{itemize}
